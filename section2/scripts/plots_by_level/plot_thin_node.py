...
# get first dataset
ucx_df = pd.read_csv('clean/thin_node_ucx.csv')
...
# get second dataset
intel_df = pd.read_csv('clean/thin_node_intel.csv')
...
# get third dataset
tcp_df = pd.read_csv('clean/thin_node_tcp.csv')
...
ax1.set_xscale('log')
ax1.set_yscale('log')
ax1.plot(bytez, ucx_times, ':^', color=cycle_colors[0], label='ucx')
ax1.plot(bytez, ucx_fit, '-', color=cycle_colors[0])
ax1.plot(bytez, intel_times, ':^', color=cycle_colors[1], label='intel')
ax1.plot(bytez, intel_fit, '-', color=cycle_colors[1])
ax1.plot(bytez, tcp_times, ':^', color=cycle_colors[2], label='self,tcp')
...
ax2.set_xscale('log')
# ax2.set_yscale('log')
ax2.plot(bytez, ucx_bw, ':^', color=cycle_colors[0], label='ucx')
ax2.plot(bytez, ucx_bwc, '-', color=cycle_colors[0])
ax2.plot(bytez, intel_bw, ':^', color=cycle_colors[1], label='intel')
ax2.plot(bytez, intel_bwc, '-', color=cycle_colors[1])
ax2.plot(bytez, tcp_bw, ':^', color=cycle_colors[2], label='self,tcp')
ax2.plot(bytez, tcp_bwc, '-', color=cycle_colors[2])
ax2.legend(loc='upper left')

f.tight_layout()
# plt.show()
plt.savefig('results/plots_by_level/plot_thin_node.png')
